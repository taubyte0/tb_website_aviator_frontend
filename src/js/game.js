...
Pilot.prototype.updateHairs = function () {
  ...
  this.angleHairs += (game.speed > 0 ? game.speed : 0.0002) * deltaTime * 40;
  //*/
};
...
CoinsHolder.prototype.spawnCoins = function () {
  var nCoins = 1 + Math.floor(rand() * 10);
  var d =
    game.seaRadius +
    game.planeDefaultHeight +
    (-1 + rand() * 2) * (game.planeAmpHeight - 20);
  var amplitude = 10 + Math.round(rand() * 10);
  for (var i = 0; i < nCoins; i++) {
    var coin;
...
export {
    game, sea, sky, gameContainer, resetGame, newGame, gameLoop, Colors
}